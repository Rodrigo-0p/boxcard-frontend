...
import SolicitudDashboard from './component/dashboard/SolicitudDashboard';
...
const SOLICITUD = () => {
    ...
    const [empresaStatus, setEmpresaStatus] = React.useState(null);
    ...
    const loadEmpresaStatus = async () => {
        setLoadingStats(true);
        try {
            const resp = await Main.Request('/base/empresa/status', 'GET');
            if (resp.data.success) {
                setEmpresaStatus(resp.data.data);
            }
        } catch (error) {
    ...
    // Detectar acción de "Nueva Solicitud" desde el Dashboard
    React.useEffect(() => {
        const params = new URLSearchParams(location.search);
        if (params.get('action') === 'new' && permisos?.globales?.create) {
            handleCreate();
        }
    }, [location.search, permisos]);
    ...
    const stats = React.useMemo(() => {
        ...
        const pendientes = solicitudesFiltradas.filter(s => s.estado === 'P');
        return {
            ...
            montoPendiente: pendientes.reduce((sum, s) => sum + (Number(s.monto_solicitado) || 0), 0),
        };
